...
/**
 * gun_az_FINAL_parsing_strategy*.csv faylından price_categories, price_positions, price_sources doldurur.
 * Mövqe kodları: gun_az_{№}. Wolt mənbələri: hər brend üçün bir sətir (3 kanal URL → source_url + fallback_urls).
 *
 * Mövqelər: `units` cədvəlindəki kanonik kodlara (g, kg, ml, l, pcs, pack, bottle) bağlanır; CSV-dəki
 * rəqəm (məs. «500 г») `unit_size`-a yazılır. Köhnə `u_*` fingerprint vahidlər yaradılmır.
 */
class GunAzParsingStrategySeeder extends Seeder
{
    private const CSV_SKIP_ROWS = 3;

    /** @var array<string, int> price_categories.code => id */
    private array $categoryIdByCode = [];

    /** @var array<string, bool> cari seed işində mövqe slug unikallığı */
    private array $usedPositionSlugs = [];
    ...
    public function run(): void
    {
        ...
        $cityId = $this->ensureBakuCity();

        DB::transaction(function () use ($path, $cityId): void {
            $this->purgePreviousGunAzData();
            $this->categoryIdByCode = [];
            $this->usedPositionSlugs = [];
    ...
    /** gun_az_151–153 → childcare_products (CSV map və ya miqrasiya əvvəli uyğunsuzluğu üçün). */
    ...
    private function ensureBakuCity(): int
    {
        ...
        if ($id !== null) {
            $id = (int) $id;
            $raw = DB::table('cities')->where('id', $id)->value('name');
            $decoded = is_string($raw) ? json_decode($raw, true) : null;
            $norm = LocalizedJson::normalizeFlatName(is_array($decoded) ? $decoded : ['az' => 'Bakı', 'en' => 'Baku', 'ru' => 'Баку']);
            if ($norm !== null) {
                DB::table('cities')->where('id', $id)->update([
                    'name' => json_encode($norm, JSON_UNESCAPED_UNICODE),
                    'updated_at' => now(),
                ]);
            }
    ...
    private function purgePreviousGunAzData(): void
    {
        ...
        DB::table('position_failures')->whereIn('position_id', $ids)->delete();
        ...
        DB::table('price_categories')->where('code', 'like', 'gun_az_cat_%')->delete();
    }

    /**
     * @param  list<string>  $data
     */
    private function importRow(array $data, int $cityId): void
    {
        ...
        $slug = $this->allocateUniqueEnglishSlugWithoutDigits($nameAz, $nameRu);

        [$unitId, $unitSize] = $this->resolveUnitColumn($unitLabel);
        $unitJson = json_encode(LocalizedJson::unitTriple('', ''), JSON_UNESCAPED_UNICODE);
        if ($unitId !== null) {
            $u = DB::table('units')->where('id', $unitId)->first();
            if ($u !== null) {
                $short = trim((string) ($u->short_name ?? ''));
                if ($short === '') {
                    $short = trim((string) ($u->code ?? ''));
                }
                $unitJson = json_encode(LocalizedJson::unitTriple($short, ''), JSON_UNESCAPED_UNICODE);
            }
        }

        $positionId = (int) DB::table('price_positions')->insertGetId([
            'city_id' => $cityId,
        ...
        foreach ($brands as $b) {
            ...
            if ($urls !== []) {
                $config['fallback_urls'] = array_values($urls);
            }
            $fallbackVariants = $this->fallbackVariants($vid, $rule);
            if ($fallbackVariants !== []) {
                $config['fallback_variants'] = $fallbackVariants;
            }
            if ($vid !== '') {
                // `price_sources.variant` sütunu silinib; variantı config-də saxlayırıq.
                $config['variant'] = Str::limit($vid, 191, '');
            }

            DB::table('price_sources')->insert([
                'position_id' => $positionId,
                'source_type' => 'wolt',
                'source_name' => Str::limit($b['label'], 120, ''),
                'source_url' => Str::limit($primary, 1024, ''),
                'source_config' => json_encode($config, JSON_UNESCAPED_UNICODE),
                'is_active' => true,
    ...
    {
        $code = $this->mapCsvCategoryTitleToLeafCode($catTitle);
        if (isset($this->categoryIdByCode[$code])) {
        ...
        $this->categoryIdByCode[$code] = (int) $id;

        return (int) $id;
    }

    /**
     * CSV vahid sütunu → `units` cədvəlindəki kanonik sətir + ölçü (məs. «500 г» → g, 500).
     * Yeni `u_*` vahidlər yaradılmır; uyğunluq yoxdursa unit_id null qalır.
     *
    ...
    /**
     * CSV kateqoriya başlığı → price_categories.code (qida/household yarpaqları və ya medicine / restaurants).
     */
    ...
    /**
     * CSV kateqoriya sütunu: əvvəldə bir və ya bir neçə emoji + boşluq ola bilər (köhnə kod yalnız bir \p{So} silirdi).
     */
    ...
    /**
     * Mövqe üçün URL slug: əsasən EN adından, rəqəmsiz; təkrar olanda təsadüfi hərf suffiksi (məs. -abcd).
     */
    private function allocateUniqueEnglishSlugWithoutDigits(string $nameAz, string $nameRu): string
    {
        $names = LocalizedJson::productName($nameAz, $nameRu);
        $base = Str::slug(trim((string) ($names['en'] ?? '')), '-', 'en');
        if ($base === '') {
            $base = Str::slug($names['az'], '-', 'az');
        }
        if ($base === '') {
            $base = Str::slug($names['ru'], '-', 'ru');
        }
        $base = $this->stripDigitsFromSlug($base);
        if ($base === '') {
            $base = 'product';
        }
        ...
        $candidate = $base;
        while (isset($this->usedPositionSlugs[$candidate])) {
            $suffix = '';
            for ($i = 0; $i < 4; $i++) {
        ...
        $this->usedPositionSlugs[$candidate] = true;

        return $candidate;
    }

    private function stripDigitsFromSlug(string $slug): string
    {
        $s = preg_replace('/\p{Nd}+/u', '', $slug) ?? $slug;
