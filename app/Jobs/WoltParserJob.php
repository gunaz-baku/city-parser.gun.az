...
class WoltParserJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        $query = DB::table('price_positions')
            ...
            ->where('price_sources.source_type', 'wolt')
                ...
                if ($hasLinksJson) {
                    $q->where(function ($q2): void {
                        $q2->whereNotNull('price_sources.links_json')
                            ...
                            ->whereRaw('CAST(price_sources.links_json AS CHAR) LIKE ?', ['%wolt.com%']);
                    });
                }
                if ($hasSourceUrl) {
                    ...
                    $q->{$outer}(function ($q2): void {
                        $q2->whereNotNull('price_sources.source_url')
                            ...
                            ->whereRaw('LOWER(price_sources.source_url) LIKE ?', ['%wolt.com%']);
                    });
                }
            })
        ...
        $max = (int) env('WOLT_PARSER_MAX_PRODUCTS', 0);
        if ($max > 0) {
            $query->limit($max);
        }
        ...
        $jobs = array_map(
            fn (int $sid) => new ParseWoltSourceJob($this->parserRunId, $sid),
            $sourceIds
        );
